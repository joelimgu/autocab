...
private:
    ...
    */
    void updateCmd(){
        ...
        { //Car started
            ...
            //Autonomous Mode
                ...
                if (!arrivedAtCurrentPoint){
                    ...
                    arrivedAtCurrentPoint = straightLine(currentLatitude, currentLongitude, (coordinates[currentPoint])[0], (coordinates[currentPoint])[1], currentDirection, requestedThrottle, reverse, requestedSteerAngle, this->get_logger(),traj_past_angle_error,traj_current_angle_error);
                    ...
                    if (!departurePointReached){
                        ...
                        arrived = straightLine(currentLatitude, currentLongitude, (coordinates[pathToDeparturePoint[pathToDeparturePoint.size()-1]])[0], (coordinates[pathToDeparturePoint[pathToDeparturePoint.size()-1]])[1], currentDirection, requestedThrottle, reverse, requestedSteerAngle, this->get_logger(),traj_past_angle_error,traj_current_angle_error);
                        if (arrived == true){
                        ...
                        arrived = straightLine(currentLatitude, currentLongitude, (coordinates[pathToFinalPoint[pathToFinalPoint.size()-1]])[0], (coordinates[pathToFinalPoint[pathToFinalPoint.size()-1]])[1], currentDirection, requestedThrottle, reverse, requestedSteerAngle, this->get_logger(),traj_past_angle_error,traj_current_angle_error);
                        if (arrived == true){
        ...
        RCLCPP_INFO(this->get_logger(), "SteerCommand after %f:",floatSteerCmd);
